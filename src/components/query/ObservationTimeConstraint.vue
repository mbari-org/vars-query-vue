...
import { useObservationTimeStore, useTimeStore } from '@/stores/query-params'
import DateTimePicker from '@/components/shared/DateTimePicker.vue'
import { onMounted, ref } from 'vue'

const timeStore = useObservationTimeStore()
    ...
    <v-container fluid style="width: 100%">
        <v-row>
            <v-col>
                <date-time-picker
                    ref="startDateRef"
                    picker-title="Start observation date"
                    time="00:00"
                    @update="updateStartTime"
                    @click:clear="onClearStart"
                >
                </date-time-picker>
            </v-col>
            ...
            <v-col>
                <date-time-picker
                    ref="endDateRef"
                    picker-title="End observation date"
                    time="23:59"
                    @update="updateEndTime"
                    @click:clear="onClearEnd"
                >
                </date-time-picker>
            </v-col>
        </v-row>
